...
test.describe('Modal Theming', () => {
    ...
    test('Collection modal uses main theme on Home Page', async ({ page }) => {
        await page.goto('/');

        // Open Collection (History) Modal
        await page.locator('.nav-results-link').click();
    ...
    test('Collection modal uses main theme on Game Page', async ({ page }) => {
        await page.goto('/');
        await page.locator('.pack-row').first().click();

        // Verify we are on a game page
        await expect(page.locator('body')).toHaveAttribute('data-theme', 'pack');
